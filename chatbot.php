...
$mqtt = new phpMQTT(“m12.cloudmqtt.com”, 1883, “phpMQTT Pub Example”); //เปลี่ยน www.yourmqttserver.com ไปที่ mqtt server ที่เราสมัครไว้นะครับ
...
if ($mqtt->connect()) {
$mqtt->publish(“/ESP/LED”,on”); // ตัวอย่างคำสั่งเปิดทีวีที่จะส่งไปยัง mqtt server
$mqtt->close();
}
$text = “เปิดไฟให้แล้วคร้าบบบบ”;
}
if (preg_match(“/ปิดไฟ/”, $text) and !preg_match(“/เปิดทีวี/”, $text)) {
if ($mqtt->connect()) {
$mqtt->publish(“/ESP/LED”,”off”);
$mqtt->close();
}
